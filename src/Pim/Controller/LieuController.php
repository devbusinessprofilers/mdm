...
final class LieuController extends AbstractController
{
    ...
    public function index(Request $request, LieuRepository $repository, SearchEngineInterface $searchEngine): Response
    {
        ...
        return $this->render('pim/lieu/index.html.twig', [
            ...
            'pending_count' => $repository->countByStatus(StatutFiche::EnAttenteValidation),
        ]);
    }
    ...
    ): Response
    {
        ...
        return $this->render('pim/lieu/show.html.twig', [
            ...
            'submit_form' => $this->transitionForm($lieu, 'submit', 'Soumettre à validation')->createView(),
            'validate_form' => $this->transitionForm($lieu, 'validate', 'Valider et publier')->createView(),
            'archive_form' => $this->transitionForm($lieu, 'archive', 'Archiver')->createView(),
    ...
    public function edit(Request $request, string $id, LieuRepository $repository, EntityManagerInterface $entityManager, OutboxPublisherInterface $outbox): Response
    {
        $lieu = $this->findLieuOrRedirect($id, $repository);
        if (!$lieu instanceof Lieu) {
            return $lieu;
    ...
    public function delete(Request $request, string $id, LieuRepository $repository, EntityManagerInterface $entityManager, OutboxPublisherInterface $outbox): Response
    {
        $lieu = $this->findLieuOrRedirect($id, $repository);
        if (!$lieu instanceof Lieu) {
            return $lieu;
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            foreach ($lieu->ressources() as $resource) {
                if (NatureRessource::Photo === $resource->nature() && '' !== $resource->damAssetId()) {
    ...
    public function submit(Request $request, string $id, LieuRepository $repository, EntityManagerInterface $entityManager, OutboxPublisherInterface $outbox): Response
    {
        $lieu = $this->requireLieu($id, $repository);
        $this->denyAccessUnlessGranted(FicheVoter::SUBMIT, $lieu->fiche());
        $form = $this->transitionForm($lieu, 'submit', 'Soumettre à validation');
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
    ...
    public function validateLieu(Request $request, string $id, LieuRepository $repository, EntityManagerInterface $entityManager, OutboxPublisherInterface $outbox): Response
    {
        $lieu = $this->requireLieu($id, $repository);
    ...
    public function reject(Request $request, string $id, LieuRepository $repository, EntityManagerInterface $entityManager, OutboxPublisherInterface $outbox): Response
    {
        $lieu = $this->requireLieu($id, $repository);
        $this->denyAccessUnlessGranted(FicheVoter::VALIDATE, $lieu->fiche());
    ...
    public function archive(Request $request, string $id, LieuRepository $repository, EntityManagerInterface $entityManager, OutboxPublisherInterface $outbox): Response
    {
        $lieu = $this->requireLieu($id, $repository);
    ...
    private function deleteForm(Lieu $lieu): \Symfony\Component\Form\FormInterface
    {
        return $this->forms->createNamed('delete_lieu', ActionType::class, null, [
            'action' => $this->generateUrl('app_pim_lieu_delete', ['id' => $lieu->id()]),
    ...
    /** @return \Symfony\Component\Form\FormInterface<mixed> */
    private function transitionForm(Lieu $lieu, string $transition, string $label): \Symfony\Component\Form\FormInterface
    {
        return $this->forms->createNamed($transition.'_lieu', ActionType::class, null, [
            'action' => $this->generateUrl('app_pim_lieu_'.$transition, ['id' => $lieu->id()]),
            'button_label' => $label,
    ...
    /** @return \Symfony\Component\Form\FormInterface<mixed> */
    private function rejectForm(Lieu $lieu): \Symfony\Component\Form\FormInterface
    {
        return $this->forms->createNamedBuilder('reject_lieu')
            ->setAction($this->generateUrl('app_pim_lieu_reject', ['id' => $lieu->id()]))
            ->setMethod('POST')
            ->add('reason', TextareaType::class, ['label' => 'Motif du refus', 'required' => true])
            ->add('submit', \Symfony\Component\Form\Extension\Core\Type\SubmitType::class, ['label' => 'Refuser'])
            ->getForm();
    }
    ...
    private function save(Request $request, Lieu $lieu, EntityManagerInterface $entityManager, OutboxPublisherInterface $outbox, bool $creation): Response
    {
        $existingMediaIds = array_map(
            ...
            array_filter($lieu->ressources()->toArray(), static fn (RessourceLieu $resource): bool => NatureRessource::Photo === $resource->nature()),
        );
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try {
                foreach ($form->get('ressources') as $resourceForm) {
                    ...
                    $outbox->enqueue(new MediaUploaded($media->id(), $media->originalStorageKey(), $media->checksum(), ImageVariantRegistry::names()));
                }
                ...
                foreach (array_diff($existingMediaIds, $currentMediaIds) as $removedMediaId) {
                    if ('' !== $removedMediaId) {
                ...
                $this->cleanupUploads($uploadedMedia);
                $form->get('ressources')->addError(new FormError(
    ...
    private function formResponse(\Symfony\Component\Form\FormInterface $form, Lieu $lieu, bool $creation): Response
    {
        ...
        foreach ($photos as &$photo) {
            ...
            $photo['metadata_form'] = $this->forms->createNamed('photo_metadata_'.$resource->id(), LieuPhotoMetadataType::class, [
                ...
                'action' => $this->generateUrl('app_pim_lieu_photo_update', ['id' => $lieu->id(), 'resourceId' => $resource->id()]),
                'method' => 'PATCH',
            ...
            $photo['replace_form'] = $this->forms->createNamed('photo_replace_'.$resource->id(), LieuPhotoReplaceType::class, null, [
                'action' => $this->generateUrl('app_pim_lieu_photo_replace', ['id' => $lieu->id(), 'resourceId' => $resource->id()]),
                'method' => 'POST',
            ])->createView();
        }
        unset($photo);
        ...
        return $this->render('pim/lieu/form.html.twig', [
            ...
            'media_upload_form' => $creation ? null : $this->forms->createNamed('lieu_photo_upload', LieuPhotoUploadType::class, null, [
                'action' => $this->generateUrl('app_pim_lieu_photo_upload', ['id' => $lieu->id()]),
                'method' => 'POST',
            ])->createView(),
    ...
    private function findLieuOrRedirect(string $id, LieuRepository $repository): Lieu|Response
    {
        $lieu = $repository->find($id);
        if ($lieu instanceof Lieu) {
            return $lieu;
        ...
        $this->addFlash('warning', 'Ce lieu n’existe plus ou vient d’être supprimé.');
