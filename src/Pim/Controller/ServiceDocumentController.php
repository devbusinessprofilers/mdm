...
#[
    Route(
        "/admin/services/{id}/documents",
        name: "app_pim_service_document_",
...
final class ServiceDocumentController extends AbstractController
{
    ...
    public function update(
        ...
        $d = $this->document($service, $resourceId);
        $form = $this->forms->createNamed(
            "service_document_metadata_" . $d->id(),
        ...
        return $this->ok($service, "Document modifié.");
    }
    ...
    public function replace(
        ...
        $d = $this->document($service, $resourceId);
        $form = $this->forms->createNamed(
            "service_document_replace_" . $d->id(),
        ...
        if (!($file instanceof UploadedFile)) {
            return $this->fail($service, "Sélectionnez un document valide.");
        }
        ...
        return $this->ok($service, "Fichier remplacé.");
    }
    ...
    public function publication(
        ...
        $d = $this->document($service, $resourceId);
        $form = $this->forms->createNamed(
            "service_document_publication_" . $d->id(),
            ActionType::class,
            null,
            ["csrf_token_id" => "service-document-publication-" . $d->id()],
        );
        $form->handleRequest($request);
        if (!$form->isSubmitted() || !$form->isValid()) {
            return $this->fail($service, "Action de publication invalide.");
        }
        try {
        ...
        return $this->ok($service, "Changement de publication mis en file.");
    }
    ...
    public function delete(
        ...
        $d = $this->document($service, $resourceId);
        $form = $this->forms->createNamed(
            "service_document_delete_" . $d->id(),
            ActionType::class,
            null,
            ["csrf_token_id" => "service-document-delete-" . $d->id()],
        );
        $form->handleRequest($request);
        if (!$form->isSubmitted() || !$form->isValid()) {
            return $this->fail($service, "Action de suppression invalide.");
        }
        $this->unpublish($d);
        $this->outbox->enqueue(new DeleteMedia($d->damAssetId()));
        $service->removeRessource($d);
        $this->em->remove($d);
        $this->changed($service);

        return $this->ok($service, "Document supprimé.");
    }
    ...
    private function ok(ServiceEvenementiel $a, string $m): RedirectResponse
    {
    ...
    private function fail(ServiceEvenementiel $a, string $m): RedirectResponse
    {
    ...
    private function back(ServiceEvenementiel $a): RedirectResponse
    {
