...
final class RestaurantDocumentController extends AbstractController
{
    ...
    public function replace(Request $request, Restaurant $restaurant, string $resourceId, RessourceLieuRepository $resources, FormFactoryInterface $forms, FicheDocumentManager $manager): RedirectResponse
    {
        $this->denyAccessUnlessGranted(FicheVoter::EDIT, $restaurant->fiche());
        $document = $resources->findDocumentForFiche($restaurant->fiche(), $resourceId);
        if (null === $document) { throw $this->createNotFoundException('Document introuvable.'); }
    ...
    public function publication(Request $request, Restaurant $restaurant, string $resourceId, RessourceLieuRepository $resources, FormFactoryInterface $forms, FicheDocumentManager $manager): RedirectResponse
    {
        $this->denyAccessUnlessGranted('ROLE_BP_VALIDATOR');
        $document = $resources->findDocumentForFiche($restaurant->fiche(), $resourceId);
        if (null === $document) { throw $this->createNotFoundException('Document introuvable.'); }
        $form = $forms->createNamed('restaurant_document_publication_'.$document->id(), ActionType::class, null, ['csrf_token_id' => 'restaurant-document-publication-'.$document->id()]);
        $form->handleRequest($request);
        if (!$form->isSubmitted() || !$form->isValid()) {
    ...
    public function delete(Request $request, Restaurant $restaurant, string $resourceId, RessourceLieuRepository $resources, FormFactoryInterface $forms, FicheDocumentManager $manager): RedirectResponse
    {
        $this->denyAccessUnlessGranted(FicheVoter::EDIT, $restaurant->fiche());
        $document = $resources->findDocumentForFiche($restaurant->fiche(), $resourceId);
        if (null === $document) { throw $this->createNotFoundException('Document introuvable.'); }
        $form = $forms->createNamed('restaurant_document_delete_'.$document->id(), ActionType::class, null, ['csrf_token_id' => 'restaurant-document-delete-'.$document->id()]);
        $form->handleRequest($request);
        if (!$form->isSubmitted() || !$form->isValid()) {
