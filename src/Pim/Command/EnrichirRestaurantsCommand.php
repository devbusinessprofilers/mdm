...
/**
 * Enrichit les restaurants (tous pays, dès qu'un GPS est présent) à partir de
 * Geoapify Place Details (tags OpenStreetMap) et en tire des suggestions à
 * arbitrer : type de cuisine, spécificités alimentaires, équipements (wifi,
 * terrasse, climatisation), accessibilité PMR, site officiel. On ne propose que
...
final class EnrichirRestaurantsCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this->addOption('appliquer', null, InputOption::VALUE_NONE, 'Crée/rafraîchit les suggestions (défaut : rapport seul).')
            ->addOption('rescan', null, InputOption::VALUE_NONE, 'Re-scanne tous les restaurants (ignore les scans récents).')
            ...
            ->addOption('batch-size', null, InputOption::VALUE_REQUIRED, 'Taille des lots de restaurants (max 1000).', '200');
    }
        ...
        do {
            ...
            foreach ($restaurants as $restaurant) {
                ...
                foreach ($propositions as $proposition) {
                    $rapport[] = [
                        'code' => (string) $restaurant->fiche()->code(),
                        'nom' => (string) $restaurant->fiche()->label(),
                        'champ' => $proposition->champ,
                        'proposition' => (string) $proposition->valeurProposee,
                    ];
                }
                if ($appliquer) {
                    $stats['suggestions créées'] += $this->enregistreur->enregistrer($restaurant->fiche(), SuggestionSource::Geoapify, $propositions);
                }
            }
            if ($appliquer) {
    ...
    private function exporterRapport(array $rapport): string
    {
        ...
        $fichier = sprintf('%s/enrichissement-restaurants-%s.csv', $dossier, date('Ymd-His'));
