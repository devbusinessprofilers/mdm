...
final class ValidateActivitesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($this->em->createQuery('SELECT a FROM '.Activite::class.' a ORDER BY a.id')->toIterable() as $a) {
            if (!$a instanceof Activite) {
                continue;
            }++$checked;
            $violations = $this->validator->validate($a, null, $groups);
            if (0 === count($violations)) {
                continue;
            }++$invalid;
            foreach ($violations as $violation) {
                $io->writeln(json_encode(['id' => $a->id(), 'code' => $a->code(), 'field' => (string) $violation->getPropertyPath(), 'error' => (string) $violation->getMessage()], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR));
            }$this->em->detach($a);
        }$io->comment(sprintf('%d activité(s) contrôlée(s), %d invalide(s). Aucune donnée modifiée.', $checked, $invalid));
