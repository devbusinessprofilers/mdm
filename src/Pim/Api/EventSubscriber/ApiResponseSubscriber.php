...
final class ApiResponseSubscriber
{
    ...
    {
        if (!str_starts_with($event->getRequest()->getPathInfo(), '/api/v1/lieux')) {
            return;
        }
        ...
        if ($exception instanceof NotNormalizableValueException) {
            $event->setResponse(new JsonResponse([
                ...
                'violations' => [['propertyPath' => $exception->getPath() ?? '', 'message' => $exception->getMessage()]],
            ], Response::HTTP_UNPROCESSABLE_ENTITY));

            return;
        }
        if ($exception instanceof NotEncodableValueException) {
            $event->setResponse(new JsonResponse(['type' => 'invalid_json', 'message' => 'Corps JSON invalide.'], Response::HTTP_BAD_REQUEST));
    ...
    public function response(ResponseEvent $event): void
    {
        ...
        if ($response->isSuccessful() && null !== ($cursor = $request->attributes->get('_api_next_cursor'))) {
            $response->headers->set('X-Next-Cursor', (string) $cursor);
        }
        if (!$response->isSuccessful() || '' === (string) $response->getContent()) {
            return;
        }
