 ...
 * (sans transition de workflow). Idempotent : collaborateur unique par email,
 * affiliation unique par (collaborateur, fiche).
 */
...
final class ImportLegacyCollaborateursCommand extends Command
{
        ...
        foreach ($reader->getSheetIterator() as $sheet) {
            foreach ($sheet->getRowIterator() as $row) {
                ...
                if (null === $columns) {
                    ...
                    if (null === $columns) {
                        $io->error('Entêtes attendues introuvables (ID Syspad, Identifiant email, Nom, Prénom, Adhérent Business Premium).');
                        $reader->close();
    ...
    {
        $wanted = [self::HEADER_SYSPAD, self::HEADER_PREMIUM, self::HEADER_EMAILS, self::HEADER_NOMS, self::HEADER_PRENOMS];
        $columns = [];
