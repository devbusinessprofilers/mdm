...
/**
 * Import manuel des services événementiels (Gamme « Prestataires de service »)
 * depuis le CSV d'export production. Idempotent via etl_legacy_fiche ;
 * code fiche = Id syspad. À exécuter avec DATABASE_URL sur la base cible.
 */
#[AsCommand(name: 'app:legacy:import-services', description: 'Importe les fiches Service depuis le CSV d\'export production.')]
final class ImportLegacyServicesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($this->reader->records($file, $headers) as $record) {
            ...
            if (null !== $onlySyspad && $row->cell(LegacyServiceRowMapper::SYSPAD_COLUMN) !== (string) $onlySyspad) {
                continue;
            }
            // Idempotence avant mapping : une relance ne remappe pas les lignes
            // déjà importées et --limit ne compte que les lignes traitées.
            if (isset($known[(int) $row->cell(LegacyServiceRowMapper::SYSPAD_COLUMN)])) {
            ...
            try {
                ...
                ++$counters['erreurs'];
                $errors[] = ['ligne' => $record->recordNumber, 'syspad' => $row->cell(LegacyServiceRowMapper::SYSPAD_COLUMN), 'message' => $exception->getMessage()];
                continue;
            }
            ...
            if (++$pendingInBatch >= $batchSize) {
                ...
                $pendingInBatch = 0;
                $io->write(sprintf("\r%d services créés…", $counters['créées']));
            }
        }
    ...
    private function persist(LegacyMappedService $mapped): void
    {
        ...
        $localisation = $service->fiche()->localisation();
        $this->entityManager->persist(new FicheSearchDocument(
            $service->fiche(),
            implode(' ', array_filter([
                (string) $service->fiche()->code(),
                $service->fiche()->label(),
                $localisation?->ville(),
