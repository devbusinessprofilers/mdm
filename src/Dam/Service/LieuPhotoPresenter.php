...
final readonly class LieuPhotoPresenter
{
    ...
    public function photos(Lieu $lieu): array
    {
        ...
        usort($resources, static fn (RessourceLieu $a, RessourceLieu $b): int => [$a->position(), $a->id()] <=> [$b->position(), $b->id()]);

        $assets = [];
        foreach ($this->mediaRepository->findByStringIds(array_map(static fn (RessourceLieu $resource): string => $resource->damAssetId(), $resources)) as $asset) {
        ...
        foreach ($resources as $resource) {
            ...
            if (null === $url && $asset instanceof MediaAsset) {
                try {
                    $url = $this->privateStorage->temporaryUrl($asset->originalStorageKey(), new \DateTimeImmutable('+10 minutes'));
                    $thumbnail = $url;
    ...
    private function renditionUrl(?MediaAsset $asset, string $name): ?string
    {
        ...
        return null === $rendition ? null : $this->publicUrl->url($rendition->storageKey());
    }
}
