...
#[ORM\UniqueConstraint(name: 'UNIQ_DAM_RENDITION_MEDIA_NAME', columns: ['media_id', 'name'])]
#[ORM\UniqueConstraint(name: 'UNIQ_DAM_RENDITION_STORAGE_KEY', columns: ['storage_key'])]
...
final class MediaRendition
{
    ...
    public function refresh(string $storageKey, int $width, int $height, int $sizeBytes): void
    {
        $this->storageKey = $storageKey;
        $this->width = $width;
        $this->height = $height;
