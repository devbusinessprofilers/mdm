...
final class PdfDocumentProcessor
{
    ...
    public function inspect(string $path): int
    {
        ...
        $process->run();
        if (!$process->isSuccessful()) { throw new \DomainException('Le PDF est illisible ou protégé.'); }
        ...
        try {
            for ($first = 1; $first <= $pages; $first += self::BATCH_SIZE) {
                ...
                if (1 === $first && $last === $pages) {
                    if (!copy($source, $output)) { throw new \RuntimeException('Impossible de préparer le PDF temporaire.'); }
                } else {
                    $pattern = $directory.'/page-%03d.pdf';
                    $split = new Process(['pdfseparate', '-f', (string) $first, '-l', (string) $last, $source, $pattern]);
                    $split->mustRun();
                    $parts = [];
                    for ($page = $first; $page <= $last; ++$page) { $parts[] = sprintf($pattern, $page); }
