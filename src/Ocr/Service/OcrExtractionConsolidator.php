...
final class OcrExtractionConsolidator
{
    ...
    public function consolidate(array $batches): array
    {
        ...
        foreach ($batches as $batch) {
            ...
            if (null === $model && is_array($response['ai_agent_info']['models'] ?? null)) {
                $names = [];
                foreach ($response['ai_agent_info']['models'] as $item) { if (is_array($item) && is_string($item['name'] ?? null)) { $names[] = $item['name']; } }
                $model = [] === $names ? null : implode(', ', array_unique($names));
            }
            foreach ($this->fields($response) as $field) {
                $path = $field['path'];
                if ($this->empty($field['value'])) { continue; }
                ...
                if (!isset($best[$path]) || $this->better($candidate, $best[$path])) { $best[$path] = $candidate; }
            }
        }
        ksort($best);
    ...
    private function fields(array $response): array
    {
        ...
        if (is_string($answer)) {
            try { $answer = json_decode($answer, true, 512, JSON_THROW_ON_ERROR); } catch (\JsonException $e) { throw new BoxProviderException('Réponse structurée Box invalide.', false, null, $e); }
        }
        if (!is_array($answer)) { throw new BoxProviderException('Réponse structurée Box invalide.'); }
        $result = [];
        ...
        foreach ($answer as $key => $answerValue) {
            ...
            if (is_array($confidenceValue)) { $confidenceValue = $confidenceValue['score'] ?? null; }
    ...
    private function pages(mixed $references): array
    {
        ...
        foreach ($references as $key => $value) {
            if (in_array($key, ['page', 'page_number'], true) && is_numeric($value) && (int) $value > 0) { $pages[] = (int) $value; continue; }
            if (is_array($value)) { $pages = [...$pages, ...$this->pages($value)]; }
        }
        return array_values(array_unique($pages));
    }

    private function empty(mixed $value): bool { return null === $value || '' === $value || [] === $value; }
