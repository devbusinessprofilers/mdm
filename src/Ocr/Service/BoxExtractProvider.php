...
final class BoxExtractProvider implements DocumentExtractionProviderInterface
{
    ...
    {
        if (!is_file($path)) {
            throw new \InvalidArgumentException('Le lot PDF temporaire est introuvable.');
        }
        ...
        if (!is_string($id) || '' === $id) {
            throw new BoxProviderException('Réponse Box upload invalide.', false);
        }
    ...
    public function delete(string $fileId): void
    {
        ...
        if (!in_array($status, [204, 404], true)) {
            $this->throwForStatus($response, 'suppression');
        }
    }
    ...
    private function token(): string
    {
        ...
        if (!is_string($token) || '' === $token) {
            throw new BoxProviderException('Réponse d’authentification Box invalide.', false);
        }
    ...
    private function json(ResponseInterface $response, string $operation): array
    {
        if ($response->getStatusCode() >= 300) {
            $this->throwForStatus($response, $operation);
        }
        try {
            $data = $response->toArray(false);
        } catch (\Throwable $e) {
            throw new BoxProviderException('Réponse JSON Box invalide pour '.$operation.'.', false, null, $e);
        }
