...
final class OcrController extends AbstractController
{
    ...
    public function retry(Request $request, Fiche $fiche, string $extractionId, DocumentExtractionRepository $repository, OcrExtractionManager $manager, OcrReviewFormFactory $forms, CurrentActorProvider $actor, OcrActions $actions): RedirectResponse
    {
        ...
        if (!$form->isSubmitted() || !$form->isValid()) {
            throw $this->createAccessDeniedException('Action de relance invalide.');
        }
        try {
            $next = $manager->retry($failed, $actor->id());
            $this->addFlash('success', 'Une nouvelle extraction a été créée.');

            return $this->redirectToRoute('app_ocr_show', ['id' => $fiche->idString(), 'extractionId' => $next->id()]);
        } catch (\DomainException $error) {
            $this->addFlash('error', $error->getMessage());

            return $this->redirectToRoute('app_ocr_show', ['id' => $fiche->idString(), 'extractionId' => $failed->id()]);
        }
    }
}
