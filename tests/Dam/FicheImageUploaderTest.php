...
final class FicheImageUploaderTest extends TestCase
{
    /** @var list<string> */
    ...
    public function testValidLieuImageIsStreamedToThePrivatePrefix(): void
    {
        ...
        $uploader = new FicheImageUploader($storage, $this->parametres(), 'test');
        $lieu = new Lieu();
        $file = $this->uploadedPng(960, 480, 'façade.png');
        $media = $uploader->upload($file, $lieu->fiche());
        ...
        self::assertSame(
            $lieu->id(),
            $storage->options['Metadata']['fiche-id'] ?? null,
        );
    }
